...
- 运行中的会话每隔 `StopPollInterval`（默认 1 秒）检查一次会话行，用来发现其他进程发来的 Stop 或丢失的锁，另外每隔几秒更新一次心跳时间。同一个 `Client` 的所有运行是**批量**检查的：每个间隔一条 `SELECT ... WHERE id IN (...)`、心跳一条 `UPDATE`，所以同时跑 500 个会话也只是每秒 1 次读，而不是 500 次。同一个 `Client` 内的 Stop 直接在内存中取消，不依赖轮询。单进程部署可以设为负数关闭轮询：其他进程发来的 Stop 会在下一步开始前生效。
