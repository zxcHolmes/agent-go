...
// Message is one chat message. Raw holds the exact JSON sent to (or received
// from) the model; it is replayed byte-for-byte so provider prefix caches stay
// valid. The other fields are decoded from Raw for convenience.
type Message struct {
	ID         string          `json:"id"`
